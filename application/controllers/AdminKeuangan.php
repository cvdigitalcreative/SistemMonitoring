...
class AdminKeuangan extends CI_Controller {
		...
		if(isset($this->session->userdata['logged_in'])and $_SESSION['logged_in']['role']==='3')
			{
				...
				$data['data']=$this->laporan_database->get_all_laporan_by_status("1");
				$this->load->view('admin/keuangan/kotakmasuk',$data);
			} else
			{
		...
		if(isset($this->session->userdata['logged_in'])and $_SESSION['logged_in']['role']==='3')
			{
				...
				$this->load->view('admin/keuangan/laporan',$data);
			} else
			{
				redirect('Home', 'refresh');
			}
		...
		if(isset($this->session->userdata['logged_in'])and $_SESSION['logged_in']['role']==='3')
			{
				...
				if(isset($_POST['ditolak'])) {
					...
					redirect('AdminKeuangan', 'refresh');
